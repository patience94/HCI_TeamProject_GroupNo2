...
def map_model_parameter(root_comp):
    for param in root_comp.modelParameters:
        if param.role=="AlongDistance":
            if param.createdBy.name=="BodyPlaneXy":
            ...
            elif param.createdBy.name=="PinPlaneXz":
                ...
                param.expression = '-param_b1+param_b/2'
            elif param.createdBy.name=="Offsetplanexz":
                param.expression = 'param_e/2'
...
def sot143(params, design = None, targetComponent = None):
    ...
    pin_plane_xz = model_3d_utility.create_offset_plane(root_comp, root_comp.xZConstructionPlane, e / 2 * ( DPins / 2 - 1 ))
    ...
    lead_slope =  0
    model_3d_sketch.create_gull_wing_lead(sketch_pin, E, 'param_E/2', E1, 'param_E1/2', body_center_z, terminal_thickness, L, lead_slope, is_parametric)
    ...
    model_3d_feature.create_mirror(root_comp, common_pin.bodies.item(0), root_comp.yZConstructionPlane)

    #odd pin 1
    extent_distance = adsk.fusion.DistanceExtentDefinition.create(adsk.core.ValueInput.createByReal(b/2))
    extent_distance1 = adsk.fusion.DistanceExtentDefinition.create(adsk.core.ValueInput.createByReal(b1-b/2))
