    ...
    for param in root_comp.modelParameters:
        if param.role=="AlongDistance":
            if param.createdBy.name=="BodyPlaneYz":      
                ...
                param.expression = '-param_L-param_L1-param_e*(param_EPins-1)+param_b*0.7'
...
def header_right_angle_socket(params, design = None, targetComponent = None):
    ...
    if is_parametric:  
        ...
        is_update = model_3d_utility.process_user_param(design, 'param_L2', L2, mm, _LCLZ("TailLengthBelowRightAngleBody", "tail length below right angle body"))
    ...
    body_slot_plane_xy = model_3d_utility.create_offset_plane(root_comp,root_comp.xZConstructionPlane, -L-L1-e*(EPins-1)+b*0.7)
    ...
    model_3d_sketch.create_center_point_rectangle(sketch_body_slot, center_point, '', '(param_E-param_e*(param_EPins-1))/2', end_point, 'param_b', 'param_b', is_parametric)
    ...
    cut_pattern = model_3d_feature.create_two_direction_pattern(root_comp, pin_socket, pattern_distance_D, DPins, root_comp.xConstructionAxis, -pattern_distance_E, EPins, root_comp.zConstructionAxis)
