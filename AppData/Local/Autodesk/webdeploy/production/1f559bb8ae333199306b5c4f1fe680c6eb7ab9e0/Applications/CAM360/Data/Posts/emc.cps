...
properties = {
  ...
  useSafeRetracts: true, // use G53 or G28 for retracting
  useWorkOffsets: true, // specifies if WCS (G54,G55,...) should be used or not
...
propertyDefinitions = {
  ...
  useG28: {title:"G28 Safe retracts", description:"Choose between using G28 or G53 for safe retracts.", type:"boolean"},
  useSafeRetracts: {title:"Use safe retracts", description:"Disable to avoid G28/G53 output for safe machine retracts. When disabled, you must manually ensure safe retracts.", type:"boolean"},
  useWorkOffsets: {title:"Use work offsets", description:"Specifies to output the WCS (eg. G54, G55)", type:"boolean"},
...
function setWorkPlane(abc) {
  ...
  if (!retracted && properties.useSafeRetracts) {
    writeRetract(Z);
  }
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    if (properties.preloadTool && properties.useToolChanger) {
  ...
  if (insertToolCall || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(gMotionModal.format(0),
        conditional(properties.useToolChanger, gFormat.format(43)), zOutput.format(initialPosition.z),
        conditional(properties.useToolChanger, hFormat.format(lengthOffset))
      );
    } else {
      writeBlock(
        gAbsIncModal.format(90),
        gMotionModal.format(0),
        conditional(properties.useToolChanger, gFormat.format(43)), xOutput.format(initialPosition.x),
        yOutput.format(initialPosition.y),
        zOutput.format(initialPosition.z), conditional(properties.useToolChanger, hFormat.format(lengthOffset))
      );
    }
