  ...
  if (isFirstCyclePoint()) {
    ...
    case "tapping":
      ...
      writeBlock(
        ...
        getCommonCycle(x, y, z, cycle.retract),
        "P" + milliFormat.format(P),
        (properties.useRigidTapping != "no") ? pitchOutput.format(tool.getThreadPitch()) : feedOutput.format(F),
        conditional((properties.useRigidTapping == "without"), sOutput.format(spindleSpeed)),
        rigidTapFormat.format((properties.useRigidTapping != "no") ? 1 : 0)
      );
      break;
    case "left-tapping":
      F = tool.getThreadPitch() * rpmFormat.getResultingValue(spindleSpeed);
