...
propertyDefinitions = {
  ...
  useChipBreaking: {title:"Use chip breaking", description:"Enable to output chip breaking cycles, the cycle will be expanded if disabled.", type:"boolean"},
  useRS274: {title:"Use RS274", description:"Enable to output RS-274 code.", type:"boolean"}
};
...
function onCyclePoint(x, y, z) {
  ...
  case "chip-breaking":
    if ((cycle.accumulatedDepth < cycle.depth) || (P > 0)) {
      expandCyclePoint(x, y, z);
    } else if (properties.useChipBreaking) {
      xOutput.reset(); // make sure at least one coordinate is output
      zOutput.reset();
      writeBlock(
        idFormat.format(0),
