...
properties = {
  ...
  fastvel: 0, // specifies a user defined rapid feedrate
  useHelicalArcs: false, // specifies if helical arcs are supported
  extendedCoolants: false // support AIR, FLOOD, and MIST coolants
};
...
propertyDefinitions = {
  ...
  forceCycleExpansion: {title:"Expand drilling cycles", description:"If enabled, all drilling cycles are expanded.", type:"boolean"},
  //optionalStop: {title:"Optional stop", description:"Outputs optional stop code during when necessary in the code.", type:"boolean"},
  fastvel: {title:"Rapid feedrate", description:"User defined rapid feedrate.", type:"number"},
  useHelicalArcs: {title:"Use helical arcs", description:"Specifies if helical arcs are supported.", type:"boolean"},
  extendedCoolants: {title:"Use extended coolant codes", description:"If enabled, Air (COOLANT), Flood (COOLANT2), and Mist (COOLANT2) coolants are supported.", type:"boolean"}
};
...
function onOpen() {
  ...
  for (var i = 0; i < getNumberOfSections(); ++i) {
    ...
    if (properties.fastvel > 0) {
      // cycleTime += section.getRapidDistance()/properties.fastvel * 60;
    }
  }
  writeComment(localize("Estimated Cycle Time") + ": " + formatCycleTime(cycleTime));
  
  if (properties.fastvel) {
    writeBlock("FASTVEL " + feedFormat.format(toVel(properties.fastvel)));
  }
...
function setCoolant(coolant) {
  ...
  switch (coolant) {
  case COOLANT_OFF:
    if (properties.extendedCoolants && (currentCoolantMode == COOLANT_FLOOD || currentCoolantMode == COOLANT_MIST)) {
      m = "COOLANT2 OFF";
    } else {
      m = "COOLANT OFF";
    }
    break;
  case COOLANT_FLOOD:
    if (properties.extendedCoolants) {
