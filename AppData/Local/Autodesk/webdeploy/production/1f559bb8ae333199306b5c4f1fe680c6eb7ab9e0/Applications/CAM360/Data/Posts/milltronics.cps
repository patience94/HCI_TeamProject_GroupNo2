...
properties = {
  ...
  G18: "unsupported", // set to "ZX" (right handed) or "XZ" (left handed) in accordance with your control setting - byte 79 - G18 plane select: XZ=0, ZX=1
  useRigidTapping: true, // output rigid tapping block
  useExtendedWCS: false, // use extended 540-599 WCS codes
...
propertyDefinitions = {
  ...
  G18: {title:"G18 plane", description:"Set to \"ZX\" (right handed) or \"XZ\" (left handed) in accordance with your control setting - byte 79 - G18 plane select: XZ=0, ZX=1.", type:"enum", values:[{id:"unsupported", title:"Unsupported"}, "ZX", "XZ"]},
  useRigidTapping: {title: "Use rigid tapping", description: "Select 'Yes' to enable G88 rigid tapping or 'No' to select G84 tapping.", type:"boolean"},
  useExtendedWCS: {title:"Use extended WCS codes", description:"If yes is selected, there are 60 available WCS, 540-590.", type:"boolean"},
  useG28: {
    title: "Safe Retracts",
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "deep-drilling":
      ...
      break;
    case "tapping":
      F = properties.useRigidTapping ? pitchOutput.format(tool.getThreadPitch()) : feedOutput.format(tool.getThreadPitch() * rpmFormat.getResultingValue(spindleSpeed));
      var tappingCode = properties.useRigidTapping ? 88 : ((tool.type == TOOL_TAP_LEFT_HAND) ? 74 : 84);
      writeBlock(
    ...
    case "left-tapping":
      F = properties.useRigidTapping ? pitchOutput.format(tool.getThreadPitch()) : feedOutput.format(tool.getThreadPitch() * rpmFormat.getResultingValue(spindleSpeed));
      var tappingCode = properties.useRigidTapping ? 88 : 74;
      writeBlock(
        gRetractModal.format(98), gAbsIncModal.format(90), gCycleModal.format(tappingCode),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + secFormat.format(P),
    ...
    case "right-tapping":
      F = properties.useRigidTapping ? pitchOutput.format(tool.getThreadPitch()) : feedOutput.format(tool.getThreadPitch() * rpmFormat.getResultingValue(spindleSpeed));
      var tappingCode = properties.useRigidTapping ? 88 : 84;
      writeBlock(
        gRetractModal.format(98), gAbsIncModal.format(90), gCycleModal.format(tappingCode),
        getCommonCycle(x, y, z, cycle.retract),
        "P" + secFormat.format(P),
