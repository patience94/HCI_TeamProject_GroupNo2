...
properties = {
  ...
  spindleDirectionCode: 4, // Can be set to 3 (M03), 4 (M04), or 34 (M03 & M04)
...
propertyDefinitions = {
  ...
  spindleDirectionCode: {title:"Spindle direction code", description:"Sets the available spindle direction code.", type:"integer", values:[{id:3, title:"M03"}, {id:4, title:"M04"}, {id:34, title:"M03 & M04"}]},
  useG28: {
    title: "Safe Retracts",
...
function onOpen() {
  ...
  if ((properties.spindleDirectionCode != 3) &&
      (properties.spindleDirectionCode != 4) &&
      (properties.spindleDirectionCode != 34)) {
...
function onSection() {
  ...
  if (insertToolCall ||
    ...
    if (properties.spindleDirectionCode == 34) {
      writeBlock(mFormat.format(tool.clockwise ? 3 : 4));
    } else {
      writeBlock(mFormat.format(properties.spindleDirectionCode));
    }
  }
