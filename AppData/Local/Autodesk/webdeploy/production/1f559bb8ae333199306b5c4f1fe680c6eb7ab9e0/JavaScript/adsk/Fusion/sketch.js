...
(function (root, factory) {
    ...
    Object.defineProperty(adsk.fusion.Sketch.prototype, 'revisionId', {
        get : function () {
    ...
    Object.defineProperty(adsk.fusion.Sketch.prototype, 'areConstraintsShown', {
        get : function () {
            var result = this._execute('areConstraintsShown');
            return result ? result.value : undefined;
        },
        set : function (value) {
            if (typeof value !== 'boolean') { throw new TypeError('value must be a boolean'); }
            ...
            var result = this._execute('areConstraintsShown', args);
            return result ? result.value : undefined;
        }
    });
    ...
    Object.defineProperty(adsk.fusion.Sketch.prototype, 'arePointsShown', {
        get : function () {
            var result = this._execute('arePointsShown');
            return result ? result.value : undefined;
        },
        set : function (value) {
            if (typeof value !== 'boolean') { throw new TypeError('value must be a boolean'); }
            ...
            var result = this._execute('arePointsShown', args);
            return result ? result.value : undefined;
        }
    });
    ...
    Object.defineProperty(adsk.fusion.Sketch.prototype, 'isLightBulbOn', {
        get : function () {
    ...
    adsk.fusion.Sketch.prototype.intersectWithSketchPlane = function (entities) {
        if (!Array.isArray(entities)) { throw new TypeError('entities must be an array'); }
        var entitiesLength = entities.length;
        var entitiesIt;
        for (entitiesIt = 0; entitiesIt < entitiesLength; ++entitiesIt) {
            if (entities[entitiesIt] !== null && !(entities[entitiesIt] instanceof adsk.core.Base)) { throw new TypeError('entities[entitiesIt] must be a adsk.core.Base'); }
        }
        var args = {
            entities : entities
        };
        var result = this._execute('intersectWithSketchPlane', args);
        ...
        for (resultIter = 0; resultIter < result.value.length; ++resultIter) {
            resultValue[resultIter] = (result.value[resultIter] !== undefined) ? adsk.createObject(result.value[resultIter], adsk.fusion.SketchEntity) : null;
        }
        return resultValue
    };
    ...
    // the faces.
    adsk.fusion.Sketch.prototype.projectToSurface = function (faces, curves, projectType, directionEntity) {
        ...
        for (curvesIt = 0; curvesIt < curvesLength; ++curvesIt) {
            if (curves[curvesIt] !== null && !(curves[curvesIt] instanceof adsk.core.Base)) { throw new TypeError('curves[curvesIt] must be a adsk.core.Base'); }
        }
        if (!isFinite(projectType)) { throw new TypeError('projectType must be a number'); }
        if (directionEntity === null || (directionEntity !== undefined && directionEntity !== null && !(directionEntity instanceof adsk.core.Base))) { throw new TypeError('directionEntity must be a adsk.core.Base'); }
        var args = {
            faces : faces,
            curves : curves,
            projectType : Number(projectType)
        };
        if (directionEntity !== undefined) {
            args.directionEntity = (directionEntity === null ? directionEntity : directionEntity.handle);
        }
        var result = this._execute('projectToSurface', args);
        if (!result || !Array.isArray(result.value)) {
            return undefined
        }
        var resultIter;
    ...
    adsk.fusion.SurfaceProjectTypes = {
    ...
    adsk.fusion.SketchFittedSpline.prototype.activateTangentHandle = function (fitPoint) {
        if (fitPoint !== null && !(fitPoint instanceof adsk.fusion.SketchPoint)) { throw new TypeError('fitPoint must be a adsk.fusion.SketchPoint'); }
        var args = {
            fitPoint : (fitPoint === null ? fitPoint : fitPoint.handle)
        };
        var result = this._execute('activateTangentHandle', args);
        return (result && result.value) ? adsk.createObject(result.value, adsk.fusion.SketchLine) : null;
    };
    ...
    adsk.fusion.SketchFittedSpline.prototype.getTangentHandle = function (fitPoint) {
        if (fitPoint !== null && !(fitPoint instanceof adsk.fusion.SketchPoint)) { throw new TypeError('fitPoint must be a adsk.fusion.SketchPoint'); }
        var args = {
            fitPoint : (fitPoint === null ? fitPoint : fitPoint.handle)
        };
        var result = this._execute('getTangentHandle', args);
        return (result && result.value) ? adsk.createObject(result.value, adsk.fusion.SketchLine) : null;
    };
    ...
    adsk.fusion.SketchFittedSpline.prototype.activateCurvatureHandle = function (fitPoint) {
        if (fitPoint !== null && !(fitPoint instanceof adsk.fusion.SketchPoint)) { throw new TypeError('fitPoint must be a adsk.fusion.SketchPoint'); }
        var args = {
            fitPoint : (fitPoint === null ? fitPoint : fitPoint.handle)
        };
        var result = this._execute('activateCurvatureHandle', args);
        return (result && result.value) ? adsk.createObject(result.value, adsk.fusion.SketchArc) : null;
    };
    ...
    adsk.fusion.SketchFittedSpline.prototype.getCurvatureHandle = function (fitPoint) {
        if (fitPoint !== null && !(fitPoint instanceof adsk.fusion.SketchPoint)) { throw new TypeError('fitPoint must be a adsk.fusion.SketchPoint'); }
        var args = {
            fitPoint : (fitPoint === null ? fitPoint : fitPoint.handle)
        };
        var result = this._execute('getCurvatureHandle', args);
        return (result && result.value) ? adsk.createObject(result.value, adsk.fusion.SketchArc) : null;
    };
