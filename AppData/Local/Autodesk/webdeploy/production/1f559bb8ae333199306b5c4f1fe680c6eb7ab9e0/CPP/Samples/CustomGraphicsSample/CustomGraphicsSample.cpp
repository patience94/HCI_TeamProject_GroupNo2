...
Ptr<ValueCommandInput> _textSize;
Ptr<ButtonRowCommandInput> _textStyle;
Ptr<StringValueCommandInput> _textFont;
...
static void changeCGObjVisibility(const std::string& strObjName)
{
	...
	if (strObjName == "Mesh") {
		...
		if (_textSize)
			_textSize->isVisible(true);
		if (_textStyle)
			_textStyle->isVisible(true);
		if (_textFont)
			_textFont->isVisible(true);
	}
	...
	void notify(const Ptr<CommandEventArgs>& eventArgs) override
	{
		...
		if (_customGraphicsObj) {
			...
			if (cgObjName == "Mesh") {
				...
				if (_text) {
					std::string font = _textFont ? _textFont->value() : "Arial";
					double size = _textSize ? _textSize->value() : 1.0;
					cgEnt = cgGroup->addText(_text->value(), font, size, Matrix3D::create());
					if (!cgEnt)
						cgEnt = cgGroup->addText(_text->value(), "Arial", size, Matrix3D::create());
					Ptr<CustomGraphicsText> cgText = cgEnt;
					if (cgText && _textStyle) {
						Ptr<ListItems> items = _textStyle->listItems();
						if (items) {
							for (size_t i = 0; i < items->count(); ++i) {
								Ptr<ListItem> item = items->item(i);
								if (!item)
									continue;
								if (item->name() == "Bold")
									cgText->isBold(item->isSelected());
								else if (item->name() == "Italic")
									cgText->isItalic(item->isSelected());
								else if (item->name() == "UnderLine")
									cgText->isUnderline(item->isSelected());
								else if (item->name() == "StrikeThrough")
									cgText->isStrikeThrough(item->isSelected());
							}
						}
					}
				}
			...
			if (cgEnt) {	
				// color effect
				if (!cgEnt->cast<CustomGraphicsPointSet>() && !cgEnt->cast<CustomGraphicsText>()) // do not apply effect to point set and text node
	...
	void notify(const Ptr<CommandCreatedEventArgs>& eventArgs) override
	{
		if (eventArgs)
		{
			...
			if (command)
			{
				...
				std::string txt = "{standard text}\\P{\\H1.5x;height:1.5x}\\P{\\C1;color:red}\\P{\\H1;height:1 unit}\\P{\\T2;char spacing:2}\\P{\\Q45;obliquing angle:45deg}\\P{\\W2;char width:2x}\\P{\\\\fArial;font:Arial}\\~{\\\\fTimes;font:Times}\\~{\\\\fCalibri;font:Calibri}\\P{\\Ooverline\\o}\\~{\\Lunderline\\l}";
				_text = inputs->addStringValueInput(_commandId + "_text", "Text", txt);
				if (_text) 
					_text->isVisible(false);
				_textSize = inputs->addValueInput(_commandId + "_textSize", "Size", "cm", ValueInput::createByReal(1));
				if (_textSize)
					_textSize->isVisible(false);
				_textStyle = inputs->addButtonRowCommandInput(_commandId + "_textStyle", "Style", true);
				if (_textStyle)
				{
					Ptr<ListItems> items = _textStyle->listItems();
					if (items)
					{
						items->add("Bold", false, "./resources/text_bold");
						items->add("Italic", false, "./resources/text_italic");
						items->add("UnderLine", false, "./resources/text_underline");
						items->add("StrikeThrough", false, "./resources/text_strikethrough");
					}
					_textStyle->isVisible(false);
				}
				_textFont = inputs->addStringValueInput(_commandId + "_textFont", "Font", "Arial");
				if (_textFont)
					_textFont->isVisible(false);
